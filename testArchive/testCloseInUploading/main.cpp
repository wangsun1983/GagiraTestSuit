        ...
        switch(msg->getEvent()) {
            case st(ArchiveMessage)::ApplyUpload: {
                if(msg->getFileName()->contains("testdata")) {
                    return ArchiveHandleResult(String::New("./tmp/upload/uploaddata"));
                }
            } break;
...
int main() {
    ...
    Md mdsum = Md::New();
    auto str1 = mdsum->encodeFile(uploadFile);
    auto str2 = mdsum->encodeFile(srcfile);

    if (!str1->sameAs(str2)) {
        TEST_FAIL("testDocuement close in uploading case2");
    }
    TEST_OK("testDocuement close in uploading case100");
