...
int main() {
    ...
    if(size != file->length()) {
        TEST_FAIL("testDocuement Query case1,size is %d,real size is %d",size,file->length());
    }
    
    size = c->querySize("aabbcc");
    if(size != 0) {
        TEST_FAIL("testDocuement Query case2,size is %d,real size is %d",size,0);
    }
    
    sleep(1);
    setEnvPort(++port);
    TEST_OK("testDocuement Query case100");
    return 0;
}
