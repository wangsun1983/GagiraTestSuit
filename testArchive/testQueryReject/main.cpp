...
int main() {
    ...
    if(size != 0) {
        TEST_FAIL("testDocuement Query Reject,size is %d,real size is %d",size,file->length());
    }
    
    if(c->getErr() != -EPERM) {
        TEST_FAIL("testDocuement Query Reject,err is %d",c->getErr());
    }
    
    sleep(1);
    setEnvPort(++port);
    TEST_OK("testDocuement Query Reject case100");
    return 0;
}
