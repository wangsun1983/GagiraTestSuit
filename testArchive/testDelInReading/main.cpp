...
int main() {
    ...
    if(ret == 0) {
        TEST_FAIL("testDocuement del in Reading case1");
    }

    File f1 = File::New(String::New("./tmp/testdata"));
    if(!f1->exists()) {
        TEST_FAIL("testDocuement del in Reading case2");
    }
    ...
    ret = connection2->remove(String::New("testdata"));
    if(ret != 0) {
        TEST_FAIL("testDocuement del in Reading case3");
    }

    File f2 = File::New("./tmp/testdata");
    if(f2->exists()) {
        TEST_FAIL("testDocuement del in Reading case4");
        return 0;
    }

    setEnvPort(++port);
    
    TEST_OK("testDocuement del in Reading case100");
    return 0;
}
