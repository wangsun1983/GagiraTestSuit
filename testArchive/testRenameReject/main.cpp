...
int main() {
    ...
    if(!check_file->exists()) {
        TEST_FAIL("testDocuement Rename Reject case1");
    }
    
    c->rename("testdata","abc_rename");
    if(!check_file->exists()) {
        TEST_FAIL("testDocuement Rename Reject case2");
    }
    
    if(c->getErr() != -EPERM) {
        TEST_FAIL("testDocuement Rename Reject,err is %d",c->getErr());
    }
    
    sleep(1);
    setEnvPort(++port);
    
    TEST_OK("testDocuement Rename Reject case100");
    return 0;
}
