...
int main() {
    ...
    int ret = c->download(String::New("testdata"),String::New("./tmp/downloaddata"));
    usleep(1000*10);
    c->close();
    usleep(1000*5000);
    setEnvPort(++port);
    ...
    if(ret != 0) {
        TEST_FAIL("testDocuement simple download case1,ret is %d",ret);
    }
    
    File downloadfile = File::New(String::New("./tmp/downloaddata"));
    if(!downloadfile->exists()) {
        TEST_FAIL("testDocuement simple download case2");
    }
    
    auto md = Md::New();
    auto md1 = md->encodeFile(File::New("./tmp/downloaddata"));
    auto md2 = md->encodeFile(File::New("./tmp/testdata"));
    
    if(!md1->sameAs(md2)) {
        TEST_FAIL("testDocuement simple download case3");
    }
    TEST_OK("testDocuement simple download case100");
    return 0;
}
