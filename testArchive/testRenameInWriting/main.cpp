...
int main() {
    ...
    if(f1->exists()) {
        TEST_FAIL("testDocuement rename in Writing case2");
    }
    ...
    ret = connection2->rename(String::New("testdata"),String::New("abc"));
    if(ret != 0) {
        TEST_FAIL("testDocuement rename in Writing case3");
    }

    File f2 = File::New("./tmp/abc");
    if(!f2->exists()) {
        TEST_FAIL("testDocuement rename in Writing case4");
        return 0;
    }

    setEnvPort(++port);
    
    TEST_OK("testDocuement rename in Writing case100");
    return 0;
}
