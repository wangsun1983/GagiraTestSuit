...
int main() {
    ...
    if(ret != -EPERM) {
        TEST_FAIL("testDocuement Open read stream reject case1,ret is %d",ret);
    }

    if(fileno != 0) {
        TEST_FAIL("testDocuement Open read stream reject case2,fileno is %d",fileno);
    }

    //input->close();
    setEnvPort(++port);
    
    TEST_OK("testDocuement Open read stream reject case100");
    return 0;
}
