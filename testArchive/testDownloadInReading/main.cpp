...
int main() {
    ...
    connection2->download(String::New("testdata"),String::New("./tmp/downloadfile"));

    sleep(1);

    File f = File::New("./tmp/downloadfile");
    if(!f->exists()) {
        TEST_FAIL("testDocuement download in Reading case1");
        return 0;
    }
    ...
    if(!str1->sameAs(str2)) {
        TEST_FAIL("testDocuement download in Reading case2");
    }

    setEnvPort(++port);
    
    TEST_OK("testDocuement download in Reading case100");
    return 0;
}
