...
int main() {
    ...
    connection->connect();
    FetchRet(fileno,ret) = connection->openStream(String::New("testdata"),O_RDONLY);
    auto input = ArchiveInputStream::New(fileno,connection);
    
    ArchiveConnection connection2 = ArchiveConnection::New(url);
    connection2->connect();
    FetchRet(fileno1,ret1) = connection2->openStream(String::New("testdata"),O_RDONLY);
    if(ret1 != 0) {
        TEST_FAIL("testDocuement Open read in reading case1,ret is %d",ret1);
    }

    if(fileno1 == 0) {
        TEST_FAIL("testDocuement Open read in reading case2,fileno is %d",fileno1);
    }

    input->close();

    FetchRet(fileno2,ret2) = connection2->openStream(String::New("testdata"),O_RDONLY);
    if(ret2 != 0) {
        TEST_FAIL("testDocuement Open read in reading case3,ret is %d",ret2);
    }

    if(fileno2 == 0) {
        TEST_FAIL("testDocuement Open read in reading case4,fileno is %d",fileno2);
    }

    setEnvPort(++port);
    
    TEST_OK("testDocuement Open read in reading case100");
    return 0;
}
