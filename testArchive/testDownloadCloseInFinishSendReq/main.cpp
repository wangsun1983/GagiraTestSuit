...
int main() {
    ...
    if(ret != -ENETUNREACH) {
        TEST_FAIL("testDocuement download close in sendReq case1,ret is %d",ret);
    }
    
    File downloadfile = File::New(String::New("./tmp/downloaddata"));
    if(downloadfile->exists()) {
        TEST_FAIL("testDocuement download close in sendReq case2");
    }
   
    TEST_OK("testDocuement download close in sendReq case100");
    return 0;
}
