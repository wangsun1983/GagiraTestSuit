...
int main() {
    ...
    auto output = ArchiveInputStream::New(fileno,connection);
    
    ArchiveConnection connection2 = ArchiveConnection::New(url);
    connection2->connect();
    ret = connection2->rename(String::New("testdata"),String::New("abc"));
    if(ret != -EBUSY) {
        TEST_FAIL("testDocuement rename in Reading case1,ret is %d",ret);
    }

    File f1 = File::New(String::New("./tmp/abc"));
    if(f1->exists()) {
        TEST_FAIL("testDocuement rename in Reading case2");
    }
    ...
    ret = connection2->rename(String::New("testdata"),String::New("abc"));
    if(ret != 0) {
        TEST_FAIL("testDocuement rename in Reading case3");
    }

    File f2 = File::New("./tmp/abc");
    if(!f2->exists()) {
        TEST_FAIL("testDocuement rename in Reading case4");
        return 0;
    }

    setEnvPort(++port);
    
    TEST_OK("testDocuement rename in Reading case100");
    return 0;
}
