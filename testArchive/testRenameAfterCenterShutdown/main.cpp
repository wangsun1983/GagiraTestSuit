...
int main() {
    ...
    if(ret != -ENETUNREACH) {
        TEST_FAIL("testDocuement Rename after shutdown case1");
    }
    
    File changeFile = File::New("./tmp/abc_rename");
    if(changeFile->exists()) {
        TEST_FAIL("testDocuement Rename after shutdown case2");
    }
    
    sleep(1);
    setEnvPort(++port);
    
    TEST_OK("testDocuement Rename after shutdown case100");
    return 0;
}
