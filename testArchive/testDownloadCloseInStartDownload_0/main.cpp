...
int main() {
    ...
    int ret = c->download(String::New("testdata"),String::New("./tmp/downloaddata"),[&c](int status,int progress){
        if(status == st(ArchiveConnection)::ProcessStatus::StartDownloading && progress == 0) {
            c->close();
        }
    });
    usleep(1000*10);
    ...
    if(ret != -ENETUNREACH) {
        TEST_FAIL("testDocuement close in download progress 0 case1,ret is %d",ret);
    }
    
    File downloadfile = File::New(String::New("./tmp/downloaddata"));
    if(downloadfile->exists()) {
        TEST_FAIL("testDocuement close in download progress 0 case2");
    }
   
    TEST_OK("testDocuement close in download progress 0 case100");
    return 0;
}
