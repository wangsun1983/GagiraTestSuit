...
int main() {
    ...
    if(!file->exists()) {
        TEST_FAIL("testDocuement Del after connection close case1");
    }
    ...
    c->close();
    if(!file->exists()) {
        TEST_FAIL("testDocuement Del after connection close case2");
    }
    usleep(1000*50);
    int ret = c->remove("testdata");
    if(ret == 0) {
        TEST_FAIL("testDocuement Del after connection close case3");
    }
    ...
    TEST_OK("testDocuement Del after connection close case100");
    return 0;
}
