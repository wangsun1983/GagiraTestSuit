...
int main() {
    ...
    if(ret != -ENETUNREACH) {
        TEST_FAIL("testDocuement upload shutdown center in SendReq case1,ret is %d",ret);
    }
    usleep(1000*10);
    c->close();
    usleep(1000*50);
    ...
    if(center->getReadLinkNums() != 0) {
        TEST_FAIL("testDocuement upload shutdown center in SendReq case2");
    }
        
    if(center->getWriteLinkNums() != 0) {
        TEST_FAIL("testDocuement upload shutdown center in SendReq case3");
    }
    ...
    if(center->getOpenLinkNums() != 0) {
        TEST_FAIL("testDocuement upload shutdown center in SendReq case4");
    }
    
    File uploadFile = File::New(String::New("./tmp/upload/uploaddata"));
    
    if(uploadFile->exists()) {
        TEST_FAIL("testDocuement upload shutdown center in SendReq case5");
    }
    
    TEST_OK("testDocuement upload shutdown center in SendReq case100");
