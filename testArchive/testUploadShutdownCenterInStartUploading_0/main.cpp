...
int main() {
    ...
    if(ret != -ENETUNREACH) {
        TEST_FAIL("testDocuement upload shutdown center in StartUploading progress 0 case1,ret is %d",ret);
    }
    usleep(1000*10);
    c->close();
    ...
    if(center->getReadLinkNums() != 0) {
        TEST_FAIL("testDocuement upload shutdown center in StartUploading progress 0 case2");
    }
        
    if(center->getWriteLinkNums() != 0) {
        TEST_FAIL("testDocuement upload shutdown center in StartUploading progress 0 case3");
    }
    ...
    if(center->getOpenLinkNums() != 0) {
        TEST_FAIL("testDocuement upload shutdown center in StartUploading progress 0 case4");
    }
    
    File uploadFile = File::New(String::New("./tmp/upload/uploaddata"));
    
    if(uploadFile->exists()) {
        TEST_FAIL("testDocuement upload shutdown center in StartUploading progress 0 case5");
    }
    
    TEST_OK("testDocuement upload shutdown center in StartUploading progress 0 case100");
