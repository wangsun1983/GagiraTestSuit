...
int main() {
    ...
    auto retval = connection->get<StudentInfo>(String::New("abc"));
    if(retval != nullptr) {
        TEST_FAIL("testMapAcquireAfterConterClose,case1");
    }
    
    if(connection->getErr() != -ESHUTDOWN) {
        TEST_FAIL("testMapAcquireAfterConterClose,case2,ret is %d",connection->getErr());
    }
    
    setEnvPort(++port);
    TEST_OK("testMapAcquireAfterConterClose,case100");
    return 0;
}
