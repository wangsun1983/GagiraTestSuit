    ...
    void onEvent(int event,String tag,ByteArray data) {
        if(event == Event::Remove) {
            ...
            if(!info->name->sameAs("wang")) {
                TEST_FAIL("testMapOnDataRemove case1,name is %s",info->name->toChars());
            }
            
            if(info->age != 1122) {
                TEST_FAIL("testMapOnDataRemove case2");
            }
            
            if(!tag->sameAs("Student")) {
                TEST_FAIL("testMapOnDataRemove case3");
            }
...
int main() {
    ...
    setEnvPort(++port);
    sleep(1);
    TEST_OK("testMapOnDataRemove case100");
    return 0;
}
