...
public:
    int onRequest(DistributeLinker linker,MapMessage msg) {
        switch(msg->getEvent()) {
            case st(MapMessage)::Type::Acquire: {
...
int main() {
    ...
    if(ret != 0) {
        TEST_FAIL("testMapAcquireReject,case1,ret is %d",ret);
    }
    
    usleep(1000*50);
    auto retvalue = connection->get<StudentInfo>(String::New("abc"));
    if(retvalue != nullptr) {
        TEST_FAIL("testMapAcquireReject,case2");
    }
    
    if(connection->getErr() != -EPERM) {
        TEST_FAIL("testMapAcquireReject,case3,ret is %d",ret);
    }
