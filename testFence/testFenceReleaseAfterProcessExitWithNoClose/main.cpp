...
int main() {
    ...
    if(pid == 0) {
        ...
        if(ret != 0) {
            TEST_FAIL("testFenceAcquire Release after process exit with no close case1,ret is %d",ret);
        }
        
        if(cost < 95 || cost > 180) {
            TEST_FAIL("testFenceAcquire Release after process exit with no close case2,cost is %d",cost);
        }
        center->close();
    }
    setEnvPort(++port);
    TEST_OK("testFenceAcquire Release after process exit with no close case100");
    return 0;
}
