...
int main() {
    ...
    Thread t1 = Thread::New([url]{
        ...
        if(cost > 20) {
            TEST_FAIL("testFenceAcquire FenceReadWriteLock ReadRealaseNotifyWrite case1,cost is %d",cost);
        }
        usleep(1000*500);
    });
    ...
    Thread t2 = Thread::New([url]{
        ...
        if(cost < 500 || cost > 530) {
            TEST_FAIL("testFenceAcquire FenceReadWriteLock ReadRealaseNotifyWrite case2,cost is %d",cost);
        }
        usleep(1000*500);
    });
    ...
    Thread t3 = Thread::New([url]{
        ...
        auto cost = watcher->stop();
        if(cost > 10020) {
            TEST_FAIL("testFenceAcquire FenceReadWriteLock ReadRealaseNotifyWrite case3,cost is %d",cost);
        }
        usleep(1000*500);
    });
    ...
    setEnvPort(++port);
    TEST_OK("testFenceAcquire FenceReadWriteLock ReadRealaseNotifyWrite case100");
    return 0;
}
